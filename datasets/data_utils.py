...
class DataSetFactory:
    def __init__(self, config):
        ...
        self.training = DataSet(transform=train_transform, samples=samples['training'], type_='training',
                                resize_shape=self.config.input_size)
        self.testing = DataSet(transform=val_transform, samples=samples['testing'], type_='testing',
                               resize_shape=self.config.input_size)
        ...
        for name in self.config.datanames.split(','):
            ...
            with open(filename, 'r') as csvin:
                ...
                for row in data:
                    ...
                    image_fn = row[1]
                    sample[
                        'image'] = image_fn if self.config.data_folder in image_fn else self.config.data_folder + image_fn
